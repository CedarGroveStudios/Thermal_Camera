...
for row in range(0, 8):
    for col in range(0, 8):
        ...
        new_element = Rect(x=pos.x, y=pos.y,
                           ...
                           fill=element_color[7 - col], outline=BLACK, stroke=2)
...
while True:
    ...
    disp_group[1].text = str(c_to_f(ALARM))
    disp_group[2].text = str(c_to_f(v_max))
    disp_group[3].text = str(c_to_f(v_sum / 64))
    disp_group[4].text = str(c_to_f(v_min))
