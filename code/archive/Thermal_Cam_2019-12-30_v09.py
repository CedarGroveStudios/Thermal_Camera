# Thermal_Cam_2019-12-30_v09.py

print("Thermal_Cam_2019-12-30_v09.py")
...
def setup_mode():
    ...
    parameter_index = 0  # index of parameter to set

    while not panel.button.select:
        left, right, up, down = move_buttons(joystick=panel.has_joystick)
        if up  : parameter_index = parameter_index - 1
        if down: parameter_index = parameter_index + 1
        if parameter_index > 2: parameter_index = 2
        if parameter_index < 0: parameter_index = 0
        disp_group[parameter_index + 66].color = BLACK
        disp_group[65].color = BLACK
        time.sleep(0.3)
        disp_group[parameter_index + 66].color = WHITE
        disp_group[65].color = WHITE
        time.sleep(0.3)
    while panel.button.select:  # wait for button release
        time.sleep(0.1)

    print(parameter_index)
    parameter_value = int(disp_group[parameter_index + 70].text)
    print(parameter_value)
    while not panel.button.select:
        left, right, up, down = move_buttons(joystick=panel.has_joystick)
        if up  :  parameter_value = parameter_value + 1
        if down:  parameter_value = parameter_value - 1
        if parameter_value > convert_temp(c=MAX_SENSOR_C):  parameter_value = convert_temp(c=MAX_SENSOR_C)
        if parameter_value < convert_temp(c=MIN_SENSOR_C):  parameter_value = convert_temp(c=MIN_SENSOR_C)
        disp_group[parameter_index + 70].text = str(parameter_value)
        disp_group[parameter_index + 66].color = GREEN
        disp_group[65].color = BLACK
        time.sleep(0.1)
        disp_group[parameter_index + 66].color = RED
        disp_group[65].color = WHITE
        time.sleep(0.1)
    while panel.button.select:  # wait for button release
        time.sleep(0.1)
...
def move_buttons(joystick=False):
    if joystick:  # for PyGamer
        move_r = move_l = False
        if   panel.joystick[0] > 50000: move_r = True
        elif panel.joystick[0] < 10000: move_l = True
        move_u = move_d = False
        if   panel.joystick[1] < 10000: move_u = True
        elif panel.joystick[1] > 50000: move_d = True
    else:  # for PyBadge
