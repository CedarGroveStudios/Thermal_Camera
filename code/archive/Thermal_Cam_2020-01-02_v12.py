# Thermal_Cam_2020-01-02_v12.py
# (c) 2020 Cedar Grove Studios

print("Thermal_Cam_2020-01-02_v12.py")
...
def update_image_frame():
    ...
    for row in range(0, 8):  # parse camera data list and update display
        for col in range(0, 8):
            ...
            image_group[((row * 8) + col) + 1] = Rect(x=pos.x, y=pos.y, width=ELEMENT_SIZE,
                                                     height=ELEMENT_SIZE,
...
def update_histo_frame():
    ...
    min_histo.text   = str(MIN_RANGE_F)
...
def setup_mode():
    ...
    ave_label.color = BLACK
    ave_value.color = BLACK
    max_value.text = str(MAX_RANGE_F)
    min_value.text = str(MIN_RANGE_F)
    time.sleep(0.8)
    status_label.text  = ""
    ...
    time.sleep(0.5)
    status_label.text = ""
    alarm_label.color = YELLOW
    alarm_value.color = YELLOW
    return int(image_group[70].text), int(image_group[71].text),int(image_group[72].text)
...
histo_group = displayio.Group(max_size=77)
board.DISPLAY.show(histo_group)
...
for row in range(0, 8):
    for col in range(0, 8):
        ...
        image_group.append(Rect(x=pos.x, y=pos.y,
