# Thermal_Cam_2020-01-13_v22.py
# (c) 2020 Cedar Grove Studios

print("Thermal_Cam_2020-01-13_v22.py")
...
def setup_mode():  # Set alarm threshold and minimum/maximum range values
    ...
    while not panel.button.start:
        ...
        while (not panel.button.a) and (not panel.button.start):
            ...
            if down:
                param_value = param_value - 1
            if param_value > celsius_to_fahrenheit(MAX_SENSOR_C):
                param_value = celsius_to_fahrenheit(MAX_SENSOR_C)
            if param_value < celsius_to_fahrenheit(MIN_SENSOR_C):
                param_value = celsius_to_fahrenheit(MIN_SENSOR_C)
...
ALARM_C     = fahrenheit_to_celsius(ALARM_F)  # alarm temp in Celsius
...
while True:
    ...
    if panel.button.select:  # toggle focus mode (focus = select button)
        ...
        if display_focus:
            ...
            MIN_RANGE_C = fahrenheit_to_celsius(MIN_RANGE_F)  # update range temp in Celsius
            MAX_RANGE_C = fahrenheit_to_celsius(MAX_RANGE_F)  # update range temp in Celsius
